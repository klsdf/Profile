 ...
 * @desc 游戏事件的构造函数，实际上就是各自成就事件，什么什么的
*/
    constructor(characterOrtachieObj = "萤雪", expression = "正常", expressionType = '正常') {
        if ('object' === typeof characterOrtachieObj) {
            ...
            this.expressionType = tachieObj.expressionType;
        } else {
     ...
     */
    updateTachie() {
        $("#ying-xue img")[0].src = `./game/assert/image/${this.character}立绘/${this.expression}/${this.expressionType}.png`
    }
}
...
$ai.keyWordCheck = function (str) {
    ...
    if( keyword.test(str) === true){
        $storyManager._storys = [
            new GameScript("我也爱主人", new Tachie("萤雪", "开心", "收到2")),
        ]
        $storyManager.playStory()
        return
    }
...
$YingXue.init = function () {
    ...
    $("#game #ying-xue").draggable({
        ...
        start: function () {
            $storyManager._storys = [
                new GameScript("啊呀，，，别晃我", new Tachie("萤雪", "不高兴", "好晕")),
            ]
        ...
        stop: function () {
            $storyManager._storys = [
                new GameScript("终于好了，", new Tachie("萤雪", "不高兴", "欸")),
            ]
            $storyManager._storyProgress = 0;
            $("#game #dialog-area").show();
            $storyManager.next()
    ...
    $('body').delegate("#game #ying-xue", 'mousedown', function (e) {
        ...
        if (1 == e.which) {
            ...
            if (seed >= 0 && seed < 20) {
                $storyManager._storys = [
                    new GameScript("欸，您这是在摸我吗？", new Tachie("萤雪", "不高兴", "欸")),
                    new GameScript("虽然我是为您服务的，但是我来这里工作也是签了保护协议的", new Tachie("萤雪", "不高兴", "欸")),
                    new GameScript("随便摸我可是会违反<stong>前端国工作法</strong>的", new Tachie("萤雪", "不高兴", "欸")),

                ]
            } else if (seed >= 20 && seed < 40) {
                $storyManager._storys = [
                    new GameScript("有点痒哦~", new Tachie("萤雪", "开心", "坏笑"))
                ]
            } else if (seed >= 40 && seed < 60) {
                $storyManager._storys = [
                    new GameScript("唔，，这么喜欢我吗？", new Tachie("萤雪", "开心", "坏笑"))
                ]
            }
            else if (seed >= 60 && seed < 80) {
                $storyManager._storys = [
                    new GameScript("欸~", new Tachie("萤雪", "不高兴", "欸"))
                ]
            }
            else if (seed >= 80 && seed < 100) {
                $storyManager._storys = [
                    new GameScript("有何贵干？", new Tachie("萤雪", "开心", "收到1"))
                ]
            }
            $storyManager._storyProgress = 0;
            $("#game #dialog-area").show();
        ...
        if (3 == e.which) {
            ...
            $storyManager._storys = [
                ...
                <input type="submit" id="打开工具网页" value="打开帽子社官网">
                <input type="submit" id="聊天" value="聊天">`, new Tachie("萤雪", "开心", "指点江山"))
            ]
            $storyManager.playStory()
            ...
            .click(function (event) {
                ...
                $storyManager._storys = [
                    new GameScript("和游戏一样，可以用代码和我聊天哦~", new Tachie("萤雪", "开心", "收到2")),
                    new GameScript("在控制台中输入$ai.speak(str)来和我聊聊吧！", new Tachie("萤雪", "开心", "淑女笑2")),
                ]
                $storyManager.playStory()
            });
            ...
            $("#打开工具网页")
                ...
                .click(function (event) {
                    event.preventDefault();
                    window.open("https://hat-soft.top/");
                });
...
$YingXue.onSchedule = function () {
    var 触发概率 = 20
    // 触发萤雪小姐姐的对话
    let seed = Math.random() * 100
    if (seed < 触发概率) {
        $storyManager._storys = [
            new GameScript("今天也要好好珍惜时间哦！", new Tachie("萤雪", "开心", "指点江山"))
        ]
        $storyManager.playStory()
    }
}
...
$YingXue.onGame = function () {
    ...
    .catch(err => {
        ...
        $storyManager._storys = [
            new GameScript("欸？刷新不了吗？", new Tachie("萤雪", "不高兴", "惊讶")),
            new GameScript("我想可能是因为我的本体没有打开吧！", new Tachie("萤雪", "开心", "淑女笑1")),
        ]
        $storyManager.playStory()
    });
}
...
$YingXue.onWordBook = function () {
    var 触发概率 = 20
    // 触发萤雪小姐姐的对话
    let seed = Math.random() * 100
    if (seed < 触发概率) {
        $storyManager._storys = [
            new GameScript("今天有好好复习单词吗~", new Tachie("萤雪", "开心", "淑女笑1"))
        ]
        $storyManager.playStory()
    }
}

$YingXue.onYingXue = function () {
    $storyManager._storys = [
        new GameScript("这个是我的房间哦~", new Tachie("萤雪", "开心", "指点江山"))
    ]
    $storyManager.playStory()
}
